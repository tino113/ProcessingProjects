...
def DLA(particles):
    ...
    while True:
        ...
        part1 = PVector(random(-1,1),random(-1,1)).normalize() * (furthestDist + outerLimit)
        part1 += centre
        quaterDiam = diam/4
        checks = 0
        innerLimit = 0
            ...
            if distance < furthestDist + outerLimit:
                ...
                for i in range(numParts-1,to,-1):
                    ...
                    if PVector.dist(part1,part2) <= diam:
                        ...
                        r = ((numParts/30 + 0  ) % 255) / 3
                        g = ((numParts/30 + 255) % 510) / 3
                        b = ((numParts/30 + 510) % 765) / 3
                        c = color(r,g,b)
                        stroke(c)
